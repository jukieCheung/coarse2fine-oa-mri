...
class OAIZIBDataset(Dataset):
    ...
    def __init__(self, excel_path, img_root, resize=None, task="kl"):
        """
        task = "kl"  : 输出 KL 0-4（五分类）
        task = "oa"  : 输出 OA 二分类标签（0/1，KL>=2 视为 OA）
        """
...
def train_one_epoch(model, loader, criterion, optimizer, device, epoch, num_classes):
    model.train()
    running_loss = 0.0
...
def main():
    ...
    train_dataset = OAIZIBDataset(args.train_excel, args.img_root,
                                resize=resize, task=task)
    test_dataset  = OAIZIBDataset(args.test_excel, args.img_root,
                                resize=resize, task=task)
